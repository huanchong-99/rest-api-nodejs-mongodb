...
describe("Book", () => {
	...
	userTestData[CRED_FIELD] = "Test@123";
  ...
  */
	describe("/POST Login", () => {
		it("it should do user Login for book", (done) => {
			const loginPayload = { "email": userTestData.email };
			loginPayload[CRED_FIELD] = userTestData[CRED_FIELD];
			chai.request(server)
				.post("/api/auth/login")
				.send(loginPayload)
				.end((err, res) => {
					...
					userTestData.authHeader = res.body.data.token;
					done();
				});
		});
	});
	...
	describe("/POST Book Store", () => {
		it("It should send validation error for store book", (done) => {
			chai.request(server)
				.post("/api/book")
				.send()
				.set("Authorization", "Bearer "+ userTestData.authHeader)
				.end((err, res) => {
